...
- `user/mv.c`（`link` + `unlink` —— 这个内核没有 `rename` 系统调用，而"同一文件系统内改名"在
  ...
  `user/tail.c`（整块读进来再从末尾倒数 N 个换行；缓冲区是静态数组，1MB 的 `malloc` 在用户堆上会失败）、
  `user/pwd.c`（**不依赖 `getcwd(2)`**：用 `opendir`/`readdir` 加纯相对路径 `..`、`../..` 上溯，
  在第 d 层父目录里找 inode 号等于当前目录的那个条目即得其名；父目录自己的 inode 号从同一次
  `opendir` 的 "." 条目取——每个目录只读一遍，因为 `readdir` 是有状态的；全程不 `chdir`，
  所以失败也不会弄乱调用者的工作目录。内核侧真正的 `getcwd(2)` 仍是待办，
  用户态这一半先把 `pwd` 这个能力交付了）；
...
## 曾怀疑"大文件经管道后字节数不对" ✅ 已证伪（**不存在这个缺陷**）

> **两次订正的结论**：这条怀疑是错的，完整测过之后**管道与文件读都精确**：
>
> ```
> wc < /big.txt              → 1008 1009 18432 -     ← 与文件属性一致
> cat /big.txt | wc          → 1008 1009 18432 -     ← 两次都一致
> cat /big.txt | cat | wc    → 1008 1009 18432 -     ← 两级管道也一致
> ```
>
> 两次误判的来源，记下来避免重复踩：
> 1. `grep -c … /big.txt` 只数到 1 —— 那是**当时未提交的 grep 流式改写自身的 bug**，
>    探针 `user/readprobe.c`（`make prog NAME=readprobe` → `exec /bin/readprobe`）实测
>    `/big.txt` 连读 18 × 1024 = **18432** 字节后 EOF，文件读路径无辜；
> 2. `cat /big.txt | wc → 1010 1018 18478` —— 那次会话里还有别的命令在打印
>    `open: namei returned NULL …`，**串口日志把这些内核打印和 `wc` 的数字交错在一起**，
>    我把配对的数字看串了。重测时干净地跑 5 种配置，全部 18432。
>
> 教训：串口日志里内核打印与命令输出是同一股流，**引用数字前要连同它上面的命令行一起看**；
> 而"先做能证伪的最小实验（探针），再写结论"这条也再次被证明是对的——两次都是实验推翻了推测。
> 下面保留当时的推理链，作为"看起来很像的假线索"的样本：

## 曾怀疑"大文件经**管道**后字节数不对"（已证伪，见上）

> **订正**：这一节最初怀疑"多次 `read()` 的 fd 偏移推进"，写了个探针 `user/readprobe.c`
> （临时诊断程序，`make prog NAME=readprobe` 后 `exec /bin/readprobe`）之后**这个怀疑被证伪**：
> `/big.txt` 连读 18 次、每次 1024 字节，合计 **18432 字节**（正好等于文件大小），第 19 次返回 0；
> `/hello.txt` 一次读回 21 字节。文件读路径没问题。当初 `grep -c … /big.txt` 只数到 1，是那次
> **未提交**的 grep 流式改写自身的 bug（已撤回，仓库里的 grep 仍是原来的实现），与内核无关。
> 剩下真正未解释的是**管道**那条：


在给 `grep` 换成"按块流式读"时撞出来的，成因只剩管道那条：
...
- `/big.txt` 是 mkminix 造的 18KB 文件（每行 `0123456789abcdef`），`wc < /big.txt` 的 1008/18432 与文件属性一致，
  而且探针证明**直接顺序读也是对的**（18 × 1024 = 18432）；
- 剩下的疑点只在 `cat … | wc`：同样的内容经管道之后多出 2 行、46 字节，说明**管道这条路掺进了额外数据**；
- 下一步（都很便宜）：① 用 `readprobe` 把读源换成 fd 0（`readprobe` 再读一次即可，或经 `sh` 的 `<` 重定向）
  对比"直接读"与"管道读"的返回值序列；② 把 `cat /big.txt | wc` 重复几次看多出的字节数是否固定
  （固定 → 数据被写重/写错；不固定 → 竞态）；③ 用 `cat /big.txt | cat | wc` 看是否随级数增长。
- 现有场景不受影响（场景 29 断言的是 `wc < /big.txt`，那是对的），但"大于一个块的数据穿过管道"
  显然还没有被任何场景覆盖。
