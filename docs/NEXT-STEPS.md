...
**判别实验**（都跑在 `exec /bin/sh` 里，同一份镜像）：

```
$ echo hello | cat      →  hello          ✅ 管道接线没问题
$ echo hello | wc       →  1 1 6 -        ✅ 6 字节的传输完全正确
$ wc < /p.txt           →  2 4 23 -       ✅ 重定向读 23 字节正确
$ cat /p.txt | wc       →  1 4 29 -       ❌ 29 > 23：读到的字节比文件还多，换行也少了
$ wc < /big.txt         →  1008 1009 18432 -   ✅ 重定向读 18KB（含间接块）正确
$ cat /big.txt | wc     →  1 4 29 -       ❌ 与上一个输入毫无关系却得到同一组数字
```

所以：**管道接线没问题，小数据（6 字节，一次写完）也没问题**；出问题的是"写者需要因为
管道满而阻塞/多次写"的传输（23 字节就已经触发）。方向基本可以锁定在
`fs/pipe.c` 的 `write_pipe()` / `read_pipe()` 与 `PIPE_SIZE`（head−tail 的模运算）、
`wake_up`/`sleep_on` 在"满了/空了"两个边界上的交错，以及读者循环对短读的处理。

**订正**：本节早先写过"场景 20（`shpipe`）把 `1 4 29 -` 当作期望值断言"——**这是错的**。
`shpipe` 只断言了 `sh: supports …`、`alpha beta gamma`、`delta`、`2 4 23 -`（重定向那条）
与 shell 正常退出，**没有**断言管道里 `wc` 的结果，所以不存在"错误值被固化"的问题；
修好之后要补的是一条**新的**断言，而不是改动已有的。
