...
| 运行（VMware / VirtualBox，D2） | `make vmdk` → `linux.vmdk`（`qemu-img convert -f raw -O vmdk -o adapter_type=ide`，monolithicSparse；`make linux-flat.vmdk` 得到 monolithicFlat），在虚拟机里挂成 **IDE 0:0（Primary Master）** 即可，**一张盘同时是内核与文件系统**。描述符带 `ddb.geometry` = 7 柱面/16 磁头/63 扇区，与内核硬编码的 CHS 换算一致。**已实测**：两个变体都在 QEMU 的 vmdk 驱动下启动成功（场景 35，1 横幅 / 0 页错误 / `MINIX: root filesystem at LBA 192` / `ls`+`cat`+`exec /bin/sh`+`ls /bin`+`wc <` 全部正常）。**未实测**：VMware/VirtualBox 自身的 BIOS 与磁盘几何——引导只用到 INT 13h AH=42h（EDD）与 IDE PIO；若某个 hypervisor 忽略 `ddb.geometry` 而按别的几何暴露磁盘，内核的 LBA→CHS 就会错位（这是真机上最该先怀疑的一点）。另外控制器必须是 **IDE**（无 SATA/AHCI/NVMe 驱动），内存 ≥16MB |
| 自动化 | `scripts/qemu-test.py` 无头驱动（串口文本 + sendkey，含大写与 `\| < > ( ) & *` 等需要 shift 的键；`--mem` 缩小客户机内存以制造压力；`--type-delay` 调打字速度；`--min-wait` 让需要观察周期性事件的用例不被“输出静止”提前收尾；`--disk` 只挂一张自启动盘（`--disk-format vmdk` 时直接启动转换后的 VMDK），D1/D2），`scripts/regress.sh` **35 个场景**（`make test`；`make test-fast` / `TEST_SKIP_HEAVY=1` 跳过 autosync/oom/evict/swap 四个重场景，CI 的 PR 跑快集），失败时发 GitHub 注解 + step summary（CI 日志要 admin 权限，注解不用），`scripts/ppm2png.py` 转截图 |
...
## 8. 内核 `printk` 支持的格式集（写诊断时必读）

内核的 `printk` 用**自己实现**的 `vsprintf`（`kernel/vsprintf.c`），与用户态 `printf`（`user/lib.c`）
是两套代码。当前支持：

- 转换：`%d` `%i` `%u` `%x` `%X` `%o` `%p` `%s` `%c`
- 长度修饰：`l`（在 i386 上 `long` 与 `int` 同宽）
- 标志：`-`（左对齐）、`0`（补零）；`+` `#` 空格被接受但无效果
- 宽度与精度：`%4d`、`%02x`、`%.3s`（截断字符串）、`%.3d`（数字补到 3 位）

**关键不变量**：标志/宽度/精度一定会在转换字符之前被解析并消费，未知转换只打印字面量、**不会吞掉
参数**。历史教训：未识别说明符（如曾经的 `%.15s`）既不打印内容又不动参数指针，导致同一条 printk 里
后续每个 `%d` 都读到前一个参数——一条"说谎的诊断"曾把整个并发 exec 的排查带偏到不存在的 inode 损坏上。
必要时用 `%s`（保证 NUL 结尾）或逐字符 `%c`，不要依赖未列出的格式。
