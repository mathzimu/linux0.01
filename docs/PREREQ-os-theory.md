...
- 此项目中：**独立的页目录 + 写时复制的用户页**（M3；此前是共享同一张页表）
...
| 机制 | 支持 | 说明 |
|------|------|------|
| fork/exit | 是 | 父子进程关系；子进程有独立地址空间（COW） |
| 管道 (pipe) | 是 | `sys_pipe`（fs/pipe.c，单页环形缓冲 + sleep_on 阻塞） |
| 信号 (signal) | 是 | `kill`/`signal`/`alarm` + **自定义处理器**（sigreturn 恢复上下文） |
| 共享内存 | 是 | fork 后父子共享只读页，写时复制（COW） |
| 消息队列 | 否 | 未实现 |
| 信号量 | 否 | 未实现 |
