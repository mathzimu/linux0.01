...
**基础应用程序**：`pwd`（当前目录，靠 `..` 反查，不依赖 `getcwd(2)`）· `cat`（读文件输出）· `head` / `tail`（前/后 N 行）· `wc`（统计行/词/字节）· `grep`（行内搜索，`-n` 行号 / `-c` 计数）· `cp`（复制文件）· `mv`（改名/移动，`link`+`unlink`）· `ln`（硬链接）· `touch`（创建空文件）· `mkdir`（建目录）· `rm`（删文件，`-r` 递归删目录）。
