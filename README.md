...
| **用户态** | `move_to_user_mode` + 嵌入程序；**`execve` 从 MINIX 加载 ELF32**（C 程序，argc/argv 传递，`exec` 命令 = fork+execve+waitpid） |
...
| **系统调用** | setup/exit/fork/read/write/open/close/getpid/pause/time/kill/sync/lseek/dup/dup2/getppid/mknod/mkdir/unlink/rmdir/waitpid/execve |
...
| 21 | `sys_execve`  | 从 MINIX 加载 ELF32 并在 Ring3 运行（LOAD 段加载、argc/argv 用户栈构造） |
